...
==========================
Generates six publication-quality plots from the sweep CSVs and prints a
summary table.  Run after the sweep completes (or at any point for partial
results).

Usage
-----
    rl-sweep plot
    python -m rlsweep.plot
"""
...
ENV_ORDER = ["CartPole-v1", "LunarLander-v3", "Acrobot-v1"]
...
def _coerce_numeric(df: pd.DataFrame) -> pd.DataFrame:
    """Convert known numeric columns to float, coercing errors to NaN."""
...
def _group_cols(df: pd.DataFrame) -> list[str]:
    """Return hyperparameter columns that are present and non-empty in df."""
    candidates = ["learning_rate", "gamma", "n_steps", "exploration_fraction"]
...
def load_results() -> pd.DataFrame:
    if not config.RUNS_CSV.exists():
        print(f"ERROR: {config.RUNS_CSV} not found. Has the sweep run?")
        sys.exit(1)

    df = pd.read_csv(config.RUNS_CSV)
    success = _coerce_numeric(df[df["status"] == "success"].copy())
...
def load_curves() -> pd.DataFrame | None:
    if not config.CURVES_CSV.exists():
        return None
    df = pd.read_csv(config.CURVES_CSV)
...
def plot_violin_distributions(df: pd.DataFrame) -> None:
    ...
    for ax, env in zip(axes, envs, strict=False):
        ...
        data = [sub[sub["algo"] == a]["mean_reward"].dropna().values for a in algos_present]
        colors = [ALGO_COLORS[a] for a in algos_present]

        vp = ax.violinplot(
            data,
            positions=range(len(algos_present)),
            showmedians=True,
            showextrema=True,
            widths=0.65,
        )
        for body, c in zip(vp["bodies"], colors, strict=False):
        ...
        for i, (algo, d) in enumerate(zip(algos_present, data, strict=False)):
            ax.scatter(
                [i],
                [d.max()],
...
def plot_seed_variance(df: pd.DataFrame) -> None:
    ...
    for ax, env in zip(axes, envs, strict=False):
        ...
        for algo in ["PPO", "DQN"]:
            ...
            if asub.empty:
                continue
            grp = asub.groupby(_group_cols(asub))["mean_reward"]
            means = grp.mean()
    ...
    fig.suptitle(
        ...
        "High sigma = result depends on initialisation, not just hyperparameters",
        fontsize=13,
        fontweight="bold",
        y=1.02,
    )
...
def plot_lr_sensitivity(df: pd.DataFrame) -> None:
    ...
    for i, algo in enumerate(algos):
        for j, env in enumerate(envs):
            ...
            ax.fill_between(lrs, means - stds, means + stds, color=color, alpha=0.18)
            ax.plot(lrs, means.values, "o-", color=color, linewidth=2.2, markersize=8)

            for lr, m in zip(lrs, means.values, strict=False):
                ax.annotate(
                    f"{m:.0f}",
                    (lr, m),
...
def plot_gamma_lr_interaction(df: pd.DataFrame) -> None:
    ...
    for ax, algo in zip(axes, algos, strict=False):
        ...
        ax.set_title(
            f"{algo} — {env}\ngamma interaction with learning rate", fontsize=11, fontweight="bold"
        )
        ax.legend(fontsize=10)

    fig.suptitle(
        "Discount Factor gamma x Learning Rate Interaction on LunarLander-v3\n"
        "gamma = 0.999 consistently outperforms gamma = 0.99 (delayed terminal reward structure)",
        fontsize=12,
        fontweight="bold",
        y=1.03,
    )
...
def plot_acrobot_collapse(df: pd.DataFrame, curves: pd.DataFrame | None) -> None:
    ...
    plot_colors = ["#d62728" if v <= -490 else "#2166ac" for v in acro_sorted["mean_reward"]]
    ax.scatter(
        range(len(acro_sorted)),
        acro_sorted["mean_reward"].values,
        c=plot_colors,
        s=75,
        alpha=0.85,
        edgecolors="white",
    ...
    ax.axhline(
        ...
        linewidth=1.5,
        alpha=0.65,
        label="Collapse floor (-500)",
    )
    ax.axhline(
        acro["mean_reward"].max(),
        color="green",
        linestyle="--",
        linewidth=1.5,
        alpha=0.65,
    ...
    ax.set_title(
        "Acrobot-v1 — All 24 PPO Runs\nRed = catastrophic collapse", fontsize=11, fontweight="bold"
    )
    ...
    if curves is not None:
        for rid, label, color, lw in [
            ...
            if not c.empty:
                ax.plot(
                    c["timestep"] / 1000, c["mean_reward"], color=color, linewidth=lw, label=label
                )
        ax.axhline(-500, color="#d62728", linestyle=":", alpha=0.4, linewidth=1)
    else:
        ax.text(
            0.5, 0.5, "curves.csv not available", ha="center", va="center", transform=ax.transAxes
        )

    ax.set_xlabel("Timestep (k)", fontsize=10)
    ax.set_ylabel("Mean Reward", fontsize=10)
    ax.set_title(
        "Identical Hyperparameters — Seed 0 vs Seed 1\ngamma=0.999, lr=3e-4, n_steps=512",
        fontsize=11,
        fontweight="bold",
    )
    ax.legend(fontsize=9)
    ...
    fig.suptitle(
        "The Acrobot Collapse: Bootstrap Cascade Under High gamma, Short Rollouts, ent_coef=0\n"
...
def plot_learning_curves(runs_df: pd.DataFrame, curves_df: pd.DataFrame) -> None:
    ...
    for i, algo in enumerate(algos):
        for j, env in enumerate(envs):
            ...
            config_cols = _group_cols(runs)
            ranked = (
                runs.groupby(config_cols)["mean_reward"]
                .mean()
            ...
            for k, (_, cfg_row) in enumerate(show.iterrows()):
                ...
                label = f"[{rank_str}] lr={lr_val:.0e} gamma={g_val}"
                color = cmap(k / max(len(show) - 1, 1))

                ax.plot(all_ts, mean_curve, color=color, linewidth=2.2, label=label)
                ax.fill_between(
                    all_ts, mean_curve - std_curve, mean_curve + std_curve, color=color, alpha=0.14
                )
...
def print_summary(df: pd.DataFrame) -> None:
    ...
    print()
    print(
        f"  {'ENV':<20} {'ALGO':<5} {'BEST':>10}  {'MEAN':>8}  {'STD':>6}  {'LR':>8}  {'GAMMA':>6}  EXTRA"
    )
    print(sep)
...
def main(argv=None) -> None:
    parser = argparse.ArgumentParser(
        description="Generate plots from sweep result CSVs.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.parse_args(argv)  # no extra args currently; accepts --help
    ...
    print_summary(df)
    print(f"\nAll plots saved to: {config.PLOTS_DIR.resolve()}\n")
