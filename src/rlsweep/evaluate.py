"""
RL Sweep — Model Evaluator
==========================
Discovers saved models from the sweep and runs evaluation episodes,
either interactively or directly by run_id.
...
-----
    ...
    # Direct by run_id
    rl-sweep evaluate --run_id CartPole-v1__PPO__gamma=0.99_learning_rate=0.001_n_steps=512__s0

    # Benchmark mode (headless, more episodes)
    rl-sweep evaluate --no_render --n_episodes 50

    # List all saved models and exit
    rl-sweep evaluate --list

    # Equivalent direct invocation
    python -m rlsweep.evaluate
"""
...
def find_models() -> list[Path]:
    ...
    return sorted(
        ...
        if p.is_dir() and (p / "best_model.zip").exists() and (p / "config.json").exists()
    )
...
def print_model_table(models: list[Path]) -> None:
    print(
        f"\n{'#':<5}  {'ENV':<18}  {'ALGO':<5}  {'SEED':<5}  {'MEAN REWARD':>11}  {'LR':>8}  {'GAMMA':>6}  RUN ID"
    )
    print("-" * 100)
...
def evaluate(
    ...
    for ep in range(1, n_episodes + 1):
        ...
        if verbose:
            print(f"  Episode {ep:>3}/{n_episodes}  |  reward={total_r:>8.2f}  |  steps={steps}")
...
def main(argv=None) -> None:
    ...
    parser.add_argument(
        "--no_render", action="store_true", help="Disable rendering (headless / benchmarking)"
    )
    parser.add_argument("--list", action="store_true", help="Print all available models and exit")
    ...
    if not models:
        ...
        print("Run rl-sweep run first, then come back here.\n")
        sys.exit(1)
